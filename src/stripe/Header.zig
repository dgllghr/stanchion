...
/// Encoding the values. This field is meaningless if the `values_len` is 0
...
pub fn read(blob: anytype) !Self {
    ...
    const values_encoding: Encoding = @enumFromInt(buf[1]);
...
pub fn write(self: Self, blob: anytype) !void {
    var buf: [encoded_len]u8 = undefined;
    buf[0] = 1;
    buf[1] = @intFromEnum(self.values_encoding);
